...
def register_animal():
    ...
    try:
        ...
        status = input(
            "Digite o status do animal [active/sold/dead]: ").strip().lower()
        # se o status for diferente dessas opções retorna erro
        if status not in ["active", "sold", "dead"]:
...
def read_animal():
    ...
    if find:
        ...
        for a in find:
            print(
                f"ID: {a['id']} | Espécie: {a['species']} | Idade: {a['age']} | Peso: {a['weight']} | Status: {a['status']}")
    else:
